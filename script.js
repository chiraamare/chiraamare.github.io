...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('mainContent') && !window.location.pathname.includes('details.html') && !window.location.pathname.includes('play.html')) {
        loadHomePage();
    }
});
    ...
    try {
        ...
        if(heroMeta) {
            heroMeta.innerHTML = `
                ...
                <span>•</span><span>${year}</span><span>•</span><span>${type}</span>
            `;
        }
    ...
    try {
        ...
        if (type === 'movie') {
            ...
            if (currentImdbId && playSection && playBtn) {
                ...
                // අලුත් Play HTML පිටුවට යවන Link එක
                const playUrl = `play.html?imdb=${currentImdbId}&type=movie`;
                
                playBtn.href = playUrl;
                playBtn.removeAttribute('target'); // අලුත් Tab එකකට යන එක නතර කරනවා
...
window.changeSeason = async function() {
    ...
    try {
        ...
        data.episodes.forEach(ep => {
            ...
            // TV Shows වල Episodes අලුත් Play HTML පිටුවට යවන Link එක
            const playUrl = currentImdbId ? `play.html?imdb=${currentImdbId}&type=tv&s=${sNum}&e=${ep.episode_number}` : '#';
